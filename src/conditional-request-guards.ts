...
export function conditionalRequestGuards(
	req: Request,
	origin_etag: string,
...
// | "1"    | "1"    | match             | match           |
// +--------+--------+-------------------+-----------------+
function strongComparision(a: string, b: string): boolean {
	return isStrongEntityTag(a) && isStrongEntityTag(b) && a === b;
}

function weakComparision(a: string, b: string): boolean {
	return a === b || opaqueTag(a) === opaqueTag(b);
}

function isWeakEntityTag(etag: string): boolean {
	return etag.startsWith('W/');
}

function isStrongEntityTag(etag: string): boolean {
...
//   If-None-Match: *
function ifNoneMatchPrecondition(
...
// If-None-Match.
function ifModifiedSincePrecondition(
...
// methods to abort a request if the selected representation does not
// match one already stored (or partially stored) from a prior request.
function ifUnmodifiedSincePrecondition(
